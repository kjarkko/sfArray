...
int main(int argc, char** argv)
{
    ...
    printf("%d\n", sa_contains(sa, "ban"));
    printf("%d\n", sa_contains(sa, "nanan"));
    
    char *s = sa_longest_recurring(sa);
    printf("%s", s);
    free(s);
...
int longest_eq_2(int a1, int a2, int b1, int b2) 
{
    return (a1 < b1 || a1 == b1 && a2 <= b2);
}

int longest_eq_3(int a1, int a2, int a3, int b1, int b2, int b3) 
{
    return (a1 < b1 || a1 == b1 && longest_eq_2(a2, a3, b2, b3));
}

void radix_pass(int *src, int *dst, int *keys, size_t len, int range) 
{
    u32 count[range+1];

    size_t i, sum;

    memset(count, 0 , (range+1) * sizeof(u32));
    for (i = 0; i <= range; i++)
        count[i] = 0;
    for (i = 0; i < len; i++)
        count[keys[src[i]]]++;
...
void DC3_sa_construct(int *values, int *SA, size_t len, size_t keyrange) 
{
    ...
    for (p = 0, t = len0 - len1, k = 0; k < len; k++) {
        i = SA12[t] < len0 ? SA12[t]*3 + 1 : (SA12[t] - len0)*3 + 2;
        j = SA0[p];
        ...
        if (SA12[t] < len0 ?
            ...
            if (p == 0)
                for (k++; t < len02; t++, k++)
                    SA[k] = SA12[t] < len0 ? SA12[t]*3 + 1 : (SA12[t] - len0)*3 + 2;
        }
    }
}
